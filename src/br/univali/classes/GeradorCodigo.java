...
public class GeradorCodigo {
    ...
    public void CriarArquivoAssembly(List<Simbolos> pListaSimbolos){
        ...
        pListaSimbolos.stream().filter(simb -> !simb.EhFuncao).forEach(simbolo ->{
    ...
    private void AdicionarInstrucaoData(Simbolos pSimbolo) {
        if (!pSimbolo.Tipo.equals("Comando")) {
            String sInstrucao = "";            
            if(!pSimbolo.EhVetor)
                sInstrucao = pSimbolo.Nome + pSimbolo.Escopo + " : " + pSimbolo.Valor;
            else
    ...
    private String EscreverValoresVetor(Simbolos pSimbolo){
        String sRetorno = pSimbolo.Nome + ": ";
